	...
	public static class CommonController
	{
		...
		public static void GrowPaintedFaces(IHull hull, Mesh sourceMesh)
		{
			...
			for (int i = 0; i < allIndices.Length; i += 3)
			{
				...
				CreateEdges(triIndex, allVertices, allIndices, out Edge e0, out Edge e1, out Edge e2);
		...
		{
			if (edges.TryGetValue(newEdge, out int count))
			{
		...
		{
			if (edges.TryGetValue(e, out int count))
			{
				return count;
			}
			else
